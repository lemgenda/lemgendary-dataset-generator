...
def build_training_notebook_content(model_key, config=None):
    ...
    accel_str = "GPU T4 x2 (Dual NVIDIA Tesla T4 - 30GB total VRAM)"
    ...
    hardware_sentinel_source = [
        ...
        "_smi_gpu = ''\n",
        "try:\n",
        "    _smi_gpu = subprocess.check_output(['nvidia-smi', '--query-gpu=name', '--format=csv,noheader'], text=True).strip()\n",
        "except Exception:\n",
        "    pass\n",
        "\n",
        "import torch\n",
        ...
        "    has_native_sm = any(f'{cap[0]}.{cap[1]}' in a or f'sm_{cap[0]}{cap[1]}' in a for a in archs)\n",
        "    \n",
        "    if 'P100' in _smi_gpu or (cap[0] < 7 and not has_native_sm):\n",
        "        print('\\n' + '=' * 78)\n",
        "        print(f'[CRITICAL INCOMPATIBILITY] {props.name} (sm_{cap[0]}{cap[1]}) is NOT supported!')\n",
        "        print('=' * 78)\n",
        "        print('Kaggle Python 3.12 / PyTorch 2.4+ has deprecated and dropped CUDA sm_60 support.')\n",
        "        print('PyTorch requires CUDA Compute Capability >= 7.0 (sm_70+).\\n')\n",
        "        print('Running on Tesla P100 will crash with: CUDA error: no kernel image is available.\\n')\n",
        "        print('[ACTION REQUIRED] Switch your Kaggle Accelerator to GPU T4 x2:')\n",
        "        print('  1. In the right sidebar under \"Session options\" -> \"Accelerator\"')\n",
        "        print('  2. Change from \"GPU P100\" to \"GPU T4 x2\"')\n",
        "        print('  3. The session will restart with Dual NVIDIA T4 GPUs (30GB total VRAM).\\n')\n",
        "        print('=' * 78 + '\\n')\n",
        "        raise RuntimeError(f'Incompatible Accelerator: {props.name} (sm_{cap[0]}{cap[1]}). Please switch to GPU T4 x2 in Kaggle Session Options.')\n",
        "    \n",
...
def build_colab_training_notebook_content(model_key, config=None):
    ...
    hardware_sentinel_source = [
        ...
        "if 'P100' in _smi_gpu:\n",
        "    print('\\n' + '=' * 78)\n",
        "    print(f'[CRITICAL INCOMPATIBILITY] {_smi_gpu} (sm_60 Pascal) is NOT supported!')\n",
        "    print('=' * 78)\n",
        "    print('PyTorch 2.4+ has deprecated and dropped CUDA sm_60 (Pascal) support.')\n",
        "    print('PyTorch requires CUDA Compute Capability >= 7.0 (sm_70+).\\n')\n",
        "    print('Running on Tesla P100 will crash with: CUDA error: no kernel image is available.\\n')\n",
        "    print('[ACTION REQUIRED] Change Runtime Accelerator to T4 GPU (or A100/L4):')\n",
        "    print('  1. In the top menu, click Runtime -> Change runtime type')\n",
        "    print('  2. Under Hardware accelerator, select T4 GPU')\n",
        "    print('  3. Click Save and reconnect.\\n')\n",
        "    print('=' * 78 + '\\n')\n",
        "    raise RuntimeError(f'Incompatible Accelerator: {_smi_gpu}. Please select T4 GPU.')\n",
        "\n",
        "import torch\n",
        "if not torch.cuda.is_available():\n",
        "    print('[WARNING] NO GPU DETECTED!')\n",
        ...
        "        raise RuntimeError(f'Incompatible Accelerator: {props.name} (sm_{cap[0]}{cap[1]}). Please select T4 GPU.')\n",
        "    print(f'[OK] [ACTIVE] {props.name} (Compute Capability sm_{cap[0]}{cap[1]})')\n",
        "    print(f'[OK] [VRAM] {props.total_memory / 1024**3:.1f} GB')\n",
        "    if torch.cuda.device_count() > 1:\n",
        "        print(f'[OK] [MULTI-GPU] Detected {torch.cuda.device_count()} active GPUs.')\n",
