...
def build_training_notebook_content(model_key, config=None):
    ...
    kaggle_accel = config.get("kaggle_accelerator", "Dual T4 or P100") if config else "Dual T4 or P100"
    ...
    hardware_sentinel_source = [
        ...
        "# --- Tesla P100 (sm_60) Native Compatibility Sentinel ---\n",
        "_is_p100 = False\n",
        "try:\n",
        "    _smi = subprocess.check_output(['nvidia-smi', '--query-gpu=name', '--format=csv,noheader'], text=True).strip()\n",
        "    if 'P100' in _smi:\n",
        "        _is_p100 = True\n",
        "except Exception:\n",
        "    pass\n",
        "\n",
        "if _is_p100:\n",
        "    _flag = '/tmp/.p100_configured'\n",
        "    _need_cu118 = False\n",
        "    try:\n",
        "        import torch\n",
        "        _archs = getattr(torch.cuda, 'get_arch_list', lambda: [])()\n",
        "        if not any('6.0' in a or 'sm_60' in a for a in _archs):\n",
        "            _need_cu118 = True\n",
        "    except Exception:\n",
        "        _need_cu118 = True\n",
        "    if _need_cu118 and not os.path.exists(_flag):\n",
        "        print('[HARDWARE] [P100 ACCELERATOR] NVIDIA Tesla P100 (sm_60 Pascal HBM2) detected.')\n",
        "        print('[HARDWARE] [P100 ACCELERATOR] Aligning PyTorch with CUDA 11.8 for native sm_60 execution...')\n",
        "        try:\n",
        "            subprocess.check_call([sys.executable, '-m', 'pip', 'install', '-q', '--no-cache-dir', 'torch', 'torchvision', 'torchaudio', '--index-url', 'https://download.pytorch.org/whl/cu118'])\n",
        "            with open(_flag, 'w') as _f: _f.write('1')\n",
        "            print('[OK] [P100 ACCELERATOR] PyTorch cu118 installed successfully.')\n",
        "            try:\n",
        "                import IPython\n",
        "                _app = IPython.Application.instance()\n",
        "                if _app and hasattr(_app, 'kernel'):\n",
        "                    print('[ACTION REQUIRED] Kernel restarted with CUDA 11.8 Pascal runtime. Re-run Cell 1!')\n",
        "                    _app.kernel.do_shutdown(True)\n",
        "            except Exception:\n",
        "                os.execv(sys.executable, [sys.executable] + sys.argv)\n",
        "        except Exception as _ie:\n",
        "            print(f'[WARNING] Auto-installation of cu118 encountered: {_ie}')\n",
        "\n",
        "import torch\n",
        ...
        "    if cap[0] == 6:\n",
        "        print(f'[OK] [P100 ACCELERATOR] Single-die Tesla P100 active (732 GB/s HBM2 memory bandwidth).')\n",
        "        print(f'[OK] [P100 ACCELERATOR] Contiguous VRAM mode engaged without multi-GPU PCIe communication bottlenecks.')\n",
        "    elif cap[0] < 6:\n",
        "        raise RuntimeError(f'Unsupported legacy GPU: {props.name} (sm_{cap[0]}{cap[1]}). Compute Capability >= 6.0 required.')\n",
...
def build_colab_training_notebook_content(model_key, config=None):
    ...
    hardware_sentinel_source = [
        ...
        "    print('[WARNING] NO GPU DETECTED!')\n",
        "    print('[ACTION REQUIRED] Enable GPU Accelerator in notebook settings:')\n",
        "    print('   -> Kaggle: Right Panel -> Session Options -> Accelerator -> GPU P100 or GPU T4 x2')\n",
        "    print('   -> Colab:  Runtime -> Change runtime type -> Hardware accelerator -> GPU')\n",
        "    print('   -> Continuing in CPU Fallback Mode for dry-run validation...')\n",
        "else:\n",
        "    props = torch.cuda.get_device_properties(0)\n",
