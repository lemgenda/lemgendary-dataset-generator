...
def generate_inference_notebook(model_key, export_dir, unified_models_registry=None, config=None):
    ...
    checkpoint_recovery_source = [
        ...
        "except Exception as kh_e: print(f'   -> [KAGGLEHUB] Model resolve notice: {kh_e}')\n",
        "\n",
        "if os.path.exists('/kaggle/input'):\n",
        "    try:\n",
        "        queue = ['/kaggle/input']\n",
...
def generate_colab_inference_notebook(model_key, export_dir, unified_models_registry=None, config=None):
    ...
    symlink_source = [
        ...
        "            try:\n",
        "                os.symlink(dest_path, a_path)\n",
        "            except OSError as exc:\n",
        "                print(f'[WARN] Could not symlink {a_path}: {exc}')\n",
        f"    if {is_forex}:\n",
        "        forex_composite_dir = os.path.join(target_dir, 'LemGendizedForexUniverseLarge')\n",
        "        os.makedirs(forex_composite_dir, exist_ok=True)\n",
        ...
        "                    try:\n",
        "                        os.symlink(os.path.join(dest_path, item), flat)\n",
        "                    except OSError as exc:\n",
        "                        print(f'[WARN] Could not symlink {flat}: {exc}')\n",
        "        link_alias = os.path.join(target_dir, 'forex')\n",
        "        if not os.path.exists(link_alias):\n",
        "            try:\n",
        "                os.symlink(dest_path, link_alias)\n",
        "            except OSError as exc:\n",
        "                print(f'[WARN] Could not symlink {link_alias}: {exc}')\n",
        "else:\n",
...
def main(argv: list[str] | None = None) -> int:
    ...
    export_root = args.output if args.output else os.path.abspath(os.path.join(proj_root, "../LemGendaryModels"))
