...
class CaptionSentry:
    ...
    def __init__(self, device="cuda" if torch.cuda.is_available() else "cpu"):
        ...
        self.model = BlipForConditionalGeneration.from_pretrained("Salesforce/blip-image-captioning-base").to(self.device)  # type: ignore
        self.model.eval()

    @torch.no_grad()
    def generate(self, img_pil):
        try:
            inputs = self.processor(img_pil, return_tensors="pt").to(self.device)  # type: ignore
