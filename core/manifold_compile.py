...
def process_dataset(parsed_args: argparse.Namespace | None = None):
    ...
    for model_key, model_config in DATASETS_META.items():
        ...
        if model_config.get("dataset_type") == "forex" or model_config.get("acquisition_mode") == "mt5_terminal":
            ...
            try:
                from forex.pipeline import run_download_pipeline
            ...
            with open(output_root / "dataset_info.yaml", "w", encoding="utf-8") as f:
                yaml.dump(yaml_info, f, default_flow_style=False, explicit_start=True)
        ...
        legacy_path = _root / ".cache" / f"registry_{pascal_name}.db"
        ...
        try:
            from tools.notebook_generator import (
                generate_training_notebook as gen_nb,
                generate_colab_training_notebook as gen_colab_nb,
            )
